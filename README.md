![Order Logo](domain/src/main/java/projectinfo/order.png)
...
- Upon creation of a customer, the following data should be provided:
    ...
    - Last name
    - Email address
    - domain.users.info.Address
    - Phone number

### Story 2: Add an item
As an admin user I want to add an item so I can expand the list of available domain.items.
...
### Story 3: Order domain.items
As a customer I want to order one or more domain.items.
...
As an admin user I want to update an item so I can keep my list of available domain.items up to date.
...
### Story 5: View report of domain.orders
As a customer I want to see a report of all my domain.orders so I can get an overview of what I ordered and how much it cost.
...
- The total price of all domain.orders
...
- A customer can only reorder one of his own domain.orders.
...
As an admin user I want to view all domain.orders that contain domain.items that should be shipped today.
...
but all domain.items are shipped to the same address.

### Story 10: Item overview
As an admin user I want to have an overview of domain.items and their stock resupply urgency so I can resupply domain.items in a timely matter.
...
- There are 3 different levels for the urgency indicator:
    ...
    - STOCK_HIGH
        -  If item amount >= 10
- The list of domain.items should be ordered based on stock resupply urgency. Starting from most urgent to less urgent.
- I should be able to provide a filter, selecting only the domain.items that have a certain level.
    - E.g.: Give me the list of domain.items with urgency indicator STOCK_LOW 
